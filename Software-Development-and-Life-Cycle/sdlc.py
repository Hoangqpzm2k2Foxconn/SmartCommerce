...
def generate_project_markdown(project_name, apps, team, template, output_dir="."):
    ...
    for i, (stage_name, _) in enumerate(template["SDLC Stages"].items()):
        stage_dir = os.path.join(
            output_dir, f"{i+1:02d}. {stage_name}"
        )  # Format: "01. Analysis"
        os.makedirs(
...
def mermaid_sdlc_diagram(template):
    """Generates a Mermaid diagram for the SDLC process with detailed info."""
    mermaid_code = "```mermaid\n"
    mermaid_code += "gantt\n"
    mermaid_code += "  dateFormat  YYYY-MM-DD\n"
    mermaid_code += "  axisFormat %Y-%m-%d\n"
    mermaid_code += "  title SDLC Overview\n\n"

    for i, (stage_name, stage_details) in enumerate(template["SDLC Stages"].items()):
        start_date = stage_details["Timeline"]["Start"].strftime("%Y-%m-%d")
        end_date = (
            stage_details["Timeline"]["End"].strftime("%Y-%m-%d")
            if isinstance(stage_details["Timeline"]["End"], datetime.date)
            else "Ongoing"
        )

        # Add the date for each step
        if end_date != "Ongoing":
            mermaid_code += f"  section {stage_name} ({start_date} to {end_date})\n"
        else:
            mermaid_code += f"  section {stage_name} (Starting: {start_date})\n"

        # Add tasks to steps
        for j, task in enumerate(stage_details["Tasks"]):
            mermaid_code += f"    {task} : {start_date}, {end_date if end_date !='Ongoing' else '1d'}\n"  # '1d' is a workaround for the Ongoing tasks

        # Roles in section
        mermaid_code += "    %% Roles: " + ", ".join(stage_details["Roles"]) + " %%\n"
...
output_directory = "SDLC_Docs"  # Output directory to save all generated files
